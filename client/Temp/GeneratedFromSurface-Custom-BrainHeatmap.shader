Shader "Custom/BrainHeatmap"
{
    Properties
    {
        ...
        _Spread ("Spread", Range(0.01, 2.0)) = 0.5 // 控制热力扩散范围
    }
    SubShader
    {
...
/* UNITY: Original start of shader */
        ...
        {
            float2 uv_MainTex;
            float3 worldPos; // 获取像素的世界坐标
            float3 localPos;
        };

        void vert (inout appdata_full v, out Input o) {
            UNITY_INITIALIZE_OUTPUT(Input, o);
            o.localPos = v.vertex.xyz; // 获取模型顶点原始坐标
        }
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            float heat = 0;
            for (int i = 0; i < 8; i++) {
                // 使用 localPos 替代 worldPos
                float d = distance(IN.localPos, _ElectrodePoints[i].xyz);
                float influence = exp(-d * d / (_Spread * _Spread));
            ...
            float3 color;
            if (heat < 0.5)
                color = lerp(cold, mid, heat * 2.0);
            else
                color = lerp(mid, hot, (heat - 0.5) * 2.0);

            o.Albedo = color;
            o.Emission = color * heat * 0.5; // 增加发光感
            o.Alpha = 1.0;
        }
